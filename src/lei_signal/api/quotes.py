...
class QuoteSnapshot:
    ...
    open: float | None = None       # 今日开盘
    high: float | None = None       # 截至当前的日内最高（盘中快照用）
    low: float | None = None        # 截至当前的日内最低
...
class TencentQuoteProvider:
    ...
    def fetch(self, symbol: str) -> QuoteSnapshot | None:
        ...
        return QuoteSnapshot(
            ...
            open=_to_float(parts[_IDX_OPEN]),
            high=_to_float(parts[_IDX_HIGH]),
            low=_to_float(parts[_IDX_LOW]),
