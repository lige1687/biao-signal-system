...
同一窗口切分（2016-01-01 起 5 个两年窗、每窗 800 根 qfq）、同一
TencentPriceProvider._parse_payload 解析、同一 validate_bars 校验。
落盘到本任务自己的池目录（不动 ~/.lei_signal_lab 共享缓存）：
...
腾讯 qfq 已做前复权，但为防送转除权残留假跳变，机械兜底规则原样执行：
任一隔夜收盘比 >1.5 或 <1/1.5 视为公司行为，把跳变日之前全部价格
（open/high/low/close 四列同乘，AK 原版仅 close、此处推广到 OHLC，
ratio 由 close 计算）乘以该比率做前复权。修正后重扫直至无 >1.5 跳变。
每只标的的 jump 清单如实写进 manifest。
...
def fetch_windowed(provider: TencentPriceProvider, symbol: str) -> pd.DataFrame:
    ...
    for start, end in WINDOWS:
        ...
        if not frame.empty:
            frames.append(frame)
        if not adjusted:
            # 个股在腾讯应有 qfq；回退 day 则中止（不冒充前复权）
            raise ValueError(f"{symbol} 腾讯未返回 qfq（ETF 回退路径不适用于个股）")
        time.sleep(SLEEP)
    if not frames:
        raise ValueError(f"{symbol} 所有窗口均无数据")
...
def main() -> None:
    ...
    for sym, name, industry, board in STOCK_POOL:
        ...
        entry = {
            ...
            "adjusted": True, "corp_action_jumps": jumps, "big_days": big,
        }
    ...
    MANIFEST.write_text(json.dumps(
        {"fetched_at": "2026-09-03", "pool": manifest,
         ...
         "adjust_rule": "AK: 隔夜收盘比 >1.5 或 <1/1.5 → 跳变日前 OHLC 同乘比率（前复权），迭代至无跳变"},
