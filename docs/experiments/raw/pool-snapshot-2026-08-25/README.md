...
### 3.1 8-31 run 与 pool 标的的差异

8-31 矩阵 12 个 run（`backtest-runs-snapshot-2026-08-31/20260831-*.json`）
的有信号标的 union = **165**，覆盖 pool 中 165/177 个标的。

**12 个标的在 pool 中但未出现在 8-31 矩阵有信号 run 里**：
...
这 12 个标的的可能是：
- 8-31 矩阵检测器扫过但无信号（被 .env 过滤 / ipo 太晚 / 数据不足）—— 11/12 个
- 8-31 之后新加的标的（无 commit 记录）—— 0/12 个候选

具体是哪一种需要对照 8-31 工作台实际跑的标的列表（已不存在）确认。
**保守结论：12 个差异标的的来源未明，不应该删**。
...
A run ma_period 分布：20=947, 60=432, 120=142。报告 1469 笔 vs run 1521 笔
的差额 52 笔很可能来自 stop-loss-matrix 复刻时只算 ma_period=20 主信号。

**8-31 矩阵 12 个 run 的 1521/563/225 是 entry_variant 全量**，
stop-loss-matrix-ARCHIVE:36-38 报告的 1469/552/224 是 stop-loss-matrix
**复刻** simulate_trade(a6_1) 的**子集**。
