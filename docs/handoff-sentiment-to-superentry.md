...
## 7. 置信度层（2026-09-06 新增，AI 引用信号时的必备依据）

- **证据账本**：`GET /api/sentiment/confidence`（源文件 `configs/sentiment_evidence.json`）
  ——每条信号的**条件胜率表**（各条件组合下的胜率/超额/样本量/p 值）、
  最佳持有期、验证状态（单年双事件/多年终审待做）、适用边界，以及
  **6 条已证伪定义清单**；
- **引用纪律**（账本 for_ai_usage 字段）：AI 引用任何信号必须同时给出
  条件胜率 + 样本量 + 验证状态 + 适用边界；禁止引用证伪清单内的定义；
  环境不满足时信号"不适用"而非降权（如非冰点环境的冰点机会=不存在，
  不是打折的冰点机会）；
- **前向存证账单**（`sentiment_signal_journal.json`，每日自动）会随时间
  积累实盘命中率——AI 引用时它应逐步取代回测数字的权重；
- 板块画像端点 `GET /api/sentiment/board/{code}` 已带 reading_cn 语义
  读法（含条件胜率引述），推荐讲解时直接引用而非自由发挥。
